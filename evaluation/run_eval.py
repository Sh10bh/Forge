...
def load_test_prompts() -> list:
    ...
    return all_prompts  # runs all 12
...
def run_evaluation(delay_between: float = 8.0) -> dict:
    """
    Runs all 12 prompts through the pipeline and collects metrics.
    delay_between: seconds to wait between requests (avoid TPM limits)
    """
    ...
    for i, test in enumerate(prompts):
        ...
        try:
            ...
            success = True
            schema_keys = [k for k in result["schema"].keys() if not k.startswith("_")]
        ...
        if i < len(prompts) - 1:
            print(f"  Waiting {delay_between}s before next prompt...")
    ...
    summary = {
        ...
        "normal_prompts": {
            ...
            "successful": sum(1 for r in normal_results if r["success"]),
        ...
        "edge_case_prompts": {
            ...
            "successful": sum(1 for r in edge_results if r["success"]),
    ...
    print(f"Normal prompts:  {summary['normal_prompts']['success_rate']}")
    print(f"Edge cases:      {summary['edge_case_prompts']['success_rate']}")
